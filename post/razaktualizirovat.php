...
$SqlQuery = "UPDATE `uchenik-zadachi`, `zadacha` SET `resheno-pravilno`=2 WHERE `uchenik-zadachi`.`id-zadachi`=`zadacha`.`id-zadachi` AND `uchenik-zadachi`.`uchenik`='".$sUchenik."' AND `uchenik-zadachi`.`predmet`='".$sPredmet."' AND (`uchenik-zadachi`.`urok`=1 OR (`resheno-pravilno`=-1 OR `razobrat-na-zanyatii`)) AND ((`zadacha`.`zadanie`='".$iZadanie."' AND `zadacha`.`sortirovka`<=".$iSortirovka.") OR `zadacha`.`zadanie`<'".$iZadanie."');";
//выполним запрос
